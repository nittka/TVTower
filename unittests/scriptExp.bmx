...
Function checkScriptTemplateVariables()
	...
	print "==================="
	endrem
	'print GetGameScriptExpression().ParseLocalizedText(title, template, TLocalization.GetLanguageID("de")).ToString()
	'print GetGameScriptExpression().ParseLocalizedText(title, template, TLocalization.GetLanguageID("en")).ToString()
	rem
	print "==================="
...
Function checkSimpleCsv()
	print "output: ~q" + GetGameScriptExpression().Parse("${.csv:~q    ~nhello;world~q   :1}").GetValueText() +"~q"
	print "output: ~q" + GetGameScriptExpression().Parse("${.csv:~q    ~nhello;world~q   :0}").GetValueText() +"~q"
	print "output: ~q" + GetGameScriptExpression().Parse("${.csv:~q    ~nhello;world~q   :0:~q;~q:0}").GetValueText() +"~q"
End Function
